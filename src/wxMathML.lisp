...
(progn
  ...
  (defun display-wxmaxima-topics (topiclist)
    ...
    (mapcar #'(lambda (&rest x) (format t "<keyword>~a</keyword>" x)) topiclist)
    (format t "</html-manual-keywords>")
    )
	  ...
	  (when (and (> (length sub-var) 0)
	    ...
	    (let* ((name-string (mstring x))
	      ...
	      (format nil  "<munder altCopy=\"~{~a~}\"><mrow>~a</mrow><mrow>~a</mrow></munder>"
		      ...
		      (format nil "<mi>~a</mi>" (wxxml-fix-string sub-var-string))
		      (format nil "<mi>~a</mi>" (wxxml-fix-string sub-string)))))))))
