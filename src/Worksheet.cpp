...
void Worksheet::OnMouseRightDown(wxMouseEvent &event) {
  ...
  if (!GetActiveCell()) {
    if (IsSelected(MC_TYPE_IMAGE) || IsSelected(MC_TYPE_SLIDE)) {
      ...
      popupMenu.Append(Event::popid_image, _("Save Image..."), wxEmptyString,
                       wxITEM_NORMAL);
      if (IsSelected(MC_TYPE_SLIDE)) {
        popupMenu.Append(Event::popid_animation_save, _("Save Animation..."),
                         wxEmptyString, wxITEM_NORMAL);
        popupMenu.Append(Event::popid_copy_animation, _("Copy Animation"),
                         wxEmptyString, wxITEM_NORMAL);
        popupMenu.Append(Event::popid_animation_start, _("Start Animation"),
                         wxEmptyString, wxITEM_NORMAL);
      } else {
        if (m_cellPointers.m_selectionStart->GetGroup()->GetGroupType() ==
            GC_TYPE_IMAGE) {
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_maxsizechooser, _("Restrict Maximum size"),
                           wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_resolutionchooser, _("Set image resolution"),
                           wxEmptyString, wxITEM_NORMAL);
	    ...
	    ->GetLabel();
	      ...
	      popupMenu.Append(Event::popid_reloadimage, _("Reload Image"),
			       wxEmptyString, wxITEM_NORMAL);
            reloadItem->Enable(false);
          } else {
            popupMenu.Append(
			     Event::popid_reloadimage,
			     wxString::Format(_("Reload Image \"%s\""), imgFile),
			     wxEmptyString, wxITEM_NORMAL);
          }

          popupMenu.Append(Event::popid_change_image, _("Change Image..."),
                           wxEmptyString, wxITEM_NORMAL);
        }
      }
      if (m_cellPointers.m_selectionStart &&
        ...
        popupMenu.AppendSeparator();
        popupMenu.Append(Event::popid_popup_gnuplot, _("Popout interactively"),
                         wxEmptyString, wxITEM_NORMAL);
      }
      ...
      if (m_cellPointers.m_selectionStart->GetType() == MC_TYPE_GROUP) {
        if (CanCopy()) {
          popupMenu.Append(wxID_COPY, _("Copy"), wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_copy_matlab, _("Copy for Octave/Matlab"),
                           wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_copy_tex, _("Copy as LaTeX"), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::Event::popid_copy_text, _("Copy as plain text"),
                           wxEmptyString, wxITEM_NORMAL);
          if (m_cellPointers.m_selectionStart == m_cellPointers.m_selectionEnd)
            popupMenu.Append(Event::popid_copy_mathml,
                             _("Copy as MathML (e.g. to word processor)"),
                             wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_copy_image, _("Copy as Image"), wxEmptyString,
                           wxITEM_NORMAL);
          if ((GetSelectionStart() != NULL) &&
            ...
            popupMenu.Append(Event::popid_copy_animation, _("Copy Animation"),
                             wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_copy_svg, _("Copy as SVG"), wxEmptyString,
                           wxITEM_NORMAL);
#if wxUSE_ENH_METAFILE
          popupMenu.Append(Event::popid_copy_emf, _("Copy as EMF"), wxEmptyString,
                           wxITEM_NORMAL);
#endif
          popupMenu.Append(Event::popid_copy_rtf, _("Copy as RTF"), wxEmptyString,
                           wxITEM_NORMAL);
          if (CanDeleteSelection())
            popupMenu.Append(Event::popid_delete, _("Delete Selection"), wxEmptyString,
                             wxITEM_NORMAL);
        }
        popupMenu.AppendSeparator();
        popupMenu.Append(Event::popid_evaluate, _("Evaluate Cell(s)"), wxEmptyString,
                         wxITEM_NORMAL);
        if (m_cellPointers.m_selectionStart == m_cellPointers.m_selectionEnd)
        ...
        if (CanMergeSelection())
          popupMenu.Append(Event::popid_merge_cells, _("Merge Cells"), wxEmptyString,
                           wxITEM_NORMAL);
        ...
        if (StartOfSectioningUnit(group)->GetGroupType() == GC_TYPE_TITLE) {
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::Event::popid_evaluate_section,
        ...
        if ((group->ContainsSavedAnswers()) ||
          ...
          popupMenu.AppendCheckItem(
				    Event::popid_auto_answer, _("Automatically send known answers"),
				    _("wxMaxima remembers answers from the last run and is able to "
				      "automatically send them to maxima, if requested"));
          popupMenu.Check(Event::popid_auto_answer, group->AutoAnswer());
          popupMenu.AppendCheckItem(
				    Event::popid_never_autoanswer, _("Never offer known answers"),
				    _("wxMaxima remembers answers from the last run and is able to "
				      "offer them as the default answer"));
          popupMenu.Check(Event::popid_never_autoanswer,
        ...
        if (IsSelected(MC_TYPE_LABEL)) {
          if (popupMenu.GetMenuItemCount() > 0)
            popupMenu.AppendSeparator();
          popupMenu.Append(Event::Event::popid_add_watch_label, _("Add to watchlist"),
                           wxEmptyString, wxITEM_NORMAL);
        }
        ...
        if ((GetSelectionStart() != NULL) &&
          ...
          if (popupMenu.GetMenuItemCount() > 0)
            popupMenu.AppendSeparator();
          popupMenu.AppendCheckItem(Event::popid_special_constant_percent,
                                    _("Show \"%\" in special constants"),
                                    wxEmptyString);
          popupMenu.Check(Event::popid_special_constant_percent,
        ...
        if ((GetSelectionStart() != NULL) &&
          ...
          if (popupMenu.GetMenuItemCount() > 0)
            popupMenu.AppendSeparator();
          popupMenu.AppendCheckItem(
				    Event::popid_hideasterisk, _("Hide multiplication dots"), wxEmptyString);
          popupMenu.Check(Event::popid_hideasterisk,
                          m_configuration->HidemultiplicationSign());
          popupMenu.AppendCheckItem(Event::popid_changeasterisk,
                                    _("Show * as multiplication dot"),
                                    wxEmptyString);
          popupMenu.Check(Event::popid_changeasterisk,
        ...
        if ((GetSelectionStart() != NULL) &&
          ...
          if (popupMenu.GetMenuItemCount() > 0)
            popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_add_watch, _("Add to watchlist"),
                           wxEmptyString, wxITEM_NORMAL);
        }

        if (IsSelected(MC_TYPE_DEFAULT) || IsSelected(MC_TYPE_LABEL)) {
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_float, _("To Float"), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_solve, _("Solve..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::Event::popid_solve_num, _("Find Root..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_simplify, _("Simplify Expression"),
                           wxEmptyString, wxITEM_NORMAL);
          popupMenu.Append(Event::popid_factor, _("Factor Expression"), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::popid_expand, _("Expand Expression"), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::popid_subst, _("Substitute..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_integrate, _("Integrate..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::popid_diff, _("Differentiate..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.AppendSeparator();
          popupMenu.Append(Event::popid_plot2d, _("Plot 2D..."), wxEmptyString,
                           wxITEM_NORMAL);
          popupMenu.Append(Event::popid_plot3d, _("Plot 3D..."), wxEmptyString,
                           wxITEM_NORMAL);
        }
      }
      if ((GetSelectionStart() == GetSelectionEnd()) &&
        ...
        popupMenu.AppendSeparator();
        popupMenu.Append(Event::popid_digits_20, _("Show max. 20 digits"));
        popupMenu.Append(Event::popid_digits_50, _("Show max. 50 digits"));
        popupMenu.Append(Event::popid_digits_100, _("Show max. 100 digits"));
        popupMenu.Append(Event::popid_digits_all, _("Always show all digits"));
        popupMenu.Append(Event::Event::popid_digits_all_linebreak,
      ...
      if (IsSelected(MC_TYPE_LABEL) || IsSelected(MC_TYPE_PROMPT) ||
        ...
        popupMenu.AppendSeparator();
        popupMenu.AppendRadioItem(Event::popid_labels_user, _("Prefer user labels"));
        popupMenu.AppendRadioItem(Event::popid_labels_autogenerated,
                                  _("Automatic labels"));
        popupMenu.AppendRadioItem(Event::Event::popid_labels_useronly, _("User labels only"));
        popupMenu.AppendRadioItem(Event::popid_labels_disable, _("Don't show labels"));
        popupMenu.Check(Event::popid_labels_autogenerated,
                        m_configuration->GetLabelChoice() ==
			Configuration::labels_automatic);
        popupMenu.Check(Event::popid_labels_user,
                        m_configuration->GetLabelChoice() ==
			Configuration::labels_prefer_user);
        popupMenu.Check(Event::Event::popid_labels_useronly,
                        m_configuration->GetLabelChoice() ==
			Configuration::labels_useronly);
        popupMenu.Check(Event::popid_labels_disable,
                        m_configuration->GetLabelChoice() ==
			Configuration::labels_none);
        wxMenu *labelWidthMenu = new wxMenu();
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth3, wxT("3 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth4, wxT("4 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth5, wxT("5 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth6, wxT("6 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth7, wxT("7 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth8, wxT("8 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth9, wxT("9 em"));
        labelWidthMenu->AppendRadioItem(Event::Event::popid_labelwidth10, wxT("10 em"));
        labelWidthMenu->Check(
			      Event::Event::popid_labelwidth3 + m_configuration->LabelWidth() - 3, true);
        popupMenu.Append(Event::popid_labelwidth, _("Label width"), labelWidthMenu);
      }
    }
      ...
      popupMenu.Append(wxID_SELECTALL, _("Select All"), wxEmptyString,
                       wxITEM_NORMAL);
      popupMenu.AppendSeparator();
      popupMenu.Append(Event::popid_insert_text, _("Insert Text Cell"), wxEmptyString,
                       wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_title, _("Insert Title Cell"),
                       wxEmptyString, wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_section, _("Insert Section Cell"),
                       wxEmptyString, wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_subsection, _("Insert Subsection Cell"),
                       wxEmptyString, wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_subsubsection,
                       _("Insert Subsubsection Cell"), wxEmptyString,
                       wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_heading5, _("Insert Heading5 Cell"),
                       wxEmptyString, wxITEM_NORMAL);
      popupMenu.Append(Event::popid_insert_heading6, _("Insert Heading6 Cell"),
                       wxEmptyString, wxITEM_NORMAL);
      popupMenu.AppendSeparator();
  ...
  // popup menu in active cell
    ...
    if (clickInSelection &&
      ...
      popupMenu.Append(Event::popid_comment_selection, _("Comment Selection"),
                       wxEmptyString, wxITEM_NORMAL);
    ...
    if (!selectionString.IsEmpty() && !selectionString.Contains("\n") &&
      ...
      popupMenu.Append(Event::popid_add_watch, _("Add to watchlist"), wxEmptyString,
                       wxITEM_NORMAL);

    if (!clickInSelection)
      popupMenu.Append(Event::popid_divide_cell, _("Divide Cell"), wxEmptyString,
                       wxITEM_NORMAL);
    ...
    if (group) {
      ...
      case GC_TYPE_CODE:
	    ...
	    m_maximaManual.GetHelpfileAnchors();
          ...
          for (unsigned int i = 0; i < m_replacementsForCurrentWord.GetCount();
               i++)
            popupMenu.Append(Event::popid_suggestion1 + i,
      ...
      case GC_TYPE_TITLE:
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Part"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Part"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      case GC_TYPE_SECTION:
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Section"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Section"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      case GC_TYPE_SUBSECTION:
        popupMenu.Append(Event::Event::popid_evaluate_section,
                         ...
                         wxEmptyString, wxITEM_NORMAL);
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Subsection"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Subsection"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      case GC_TYPE_SUBSUBSECTION:
        popupMenu.Append(Event::Event::popid_evaluate_section,
                         ...
                         wxEmptyString, wxITEM_NORMAL);
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Subsubsection"),
                           wxEmptyString, wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Subsubsection"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      case GC_TYPE_HEADING5:
        popupMenu.Append(Event::Event::popid_evaluate_section,
                         ...
                         wxEmptyString, wxITEM_NORMAL);
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Heading 5"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Heading 5"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      case GC_TYPE_HEADING6:
        popupMenu.Append(Event::Event::popid_evaluate_section,
                         ...
                         wxEmptyString, wxITEM_NORMAL);
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide Heading 6"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide Heading 6"), wxEmptyString,
                           wxITEM_NORMAL);
        break;
      default:
        if (group->GetHiddenTree() != NULL)
          popupMenu.Append(Event::popid_unfold, _("Unhide contents"), wxEmptyString,
                           wxITEM_NORMAL);
        else
          popupMenu.Append(Event::popid_fold, _("Hide contents"), wxEmptyString,
                           wxITEM_NORMAL);
      }
    }
    ...
    if (GetActiveCell()) {
      ...
      if ((selectionStyle == TS_CODE_VARIABLE) ||
        ...
        if ((selectionStyle != TS_FUNCTION) && (selectionStyle != TS_STRING)) {
          facts_sub->Append(Event::popid_property_real, _("Is a real variable"),
                            _("This symbol has no imaginary part"));
          facts_sub->Append(
			    Event::popid_property_complex, _("Is a complex variable"),
			    _("This symbol might have both real and imaginary part"));
          facts_sub->Append(Event::popid_property_imaginary,
                            _("Is an imaginary variable"),
                            _("This symbol has no real part"));
          facts_sub->Append(Event::popid_property_even, _("Even number"));
          facts_sub->Append(Event::popid_property_odd, _("Odd number"));
          facts_sub->Append(Event::popid_property_integer,
                            _("Is an integer variable"));
          facts_sub->Append(Event::popid_property_noninteger,
                            _("Is no integer variable"));
          facts_sub->Append(Event::popid_property_rational, _("A rational variable"));
          facts_sub->Append(Event::popid_property_irrational,
                            _("A irrational variable"));
          facts_sub->Append(Event::popid_property_greaterThan,
                            _("Greater or less than a value"),
                            _("Calls assume() in order to tell maxima about "
                              "the variable's range"));
          facts_sub->AppendSeparator();
          facts_sub->Append(Event::popid_property_constant, _("Symbolic Constant"));
          facts_sub->Append(Event::popid_property_nonarray, _("No Array"));
          facts_sub->Append(Event::popid_property_scalar, _("A Scalar"));
          facts_sub->Append(Event::popid_property_nonscalar, _("No Scalar"));
          facts_sub->Append(Event::popid_property_mainvar,
                            _("Likely to be the main variable"));
          facts_sub->AppendSeparator();
          facts_sub->Append(Event::popid_property_bindtest,
        ...
        if ((selectionStyle == TS_CODE_VARIABLE) ||
          ...
          facts_sub->Append(
			    Event::popid_property_atvalue, _("Value at a specific point"),
			    _("Inform maxima about the value of the function at a specific "
			      "point, e.G. for declaring initial conditions"));
          facts_sub->AppendSeparator();
          facts_sub->Append(Event::Event::popid_property_evenfun, _("Is an even function"),
                            _("This function will return even integers"));
          facts_sub->Append(Event::Event::popid_property_oddfun, _("Is an odd function"),
                            _("This function will return odd integers"));
          facts_sub->Append(Event::popid_property_additive,
                            _("Additive function: f(a+b)=f(a)+f(b)"));
          facts_sub->Append(Event::popid_property_multiplicative,
                            _("Multiplicative function: f(a*b)=f(a)*f(b)"));
          facts_sub->Append(Event::popid_property_nary,
                            _("nary: f(x, f(y,z)) = foo(x,y,z)"));
          facts_sub->Append(Event::popid_property_antisymmetric,
                            _("Antisymmetric function: f(x)=-f(-x)"));
          facts_sub->Append(Event::popid_property_commutative,
                            _("Commutative function"));
          facts_sub->Append(Event::popid_property_symmetric,
                            _("Symmetric function: f(x)=f(-x)"));
          facts_sub->Append(Event::popid_property_increasing,
                            _("Increasing function f(x)>x"));
          facts_sub->Append(Event::popid_property_decreasing,
                            _("Decreasing function f(x)<x"));
          facts_sub->Append(Event::Event::popid_property_integervalued,
                            _("A function returning integers"));
          facts_sub->Append(Event::popid_property_posfun,
                            _("A function returning positive numbers"));
          facts_sub->Append(Event::popid_property_lassociative,
                            _("A left-associative function"));
          facts_sub->Append(Event::popid_property_rassociative,
                            _("A right-associative function"));
          facts_sub->Append(Event::popid_property_linear, _("A linear function"));
          facts_sub->Append(Event::popid_property_outative,
                            _("outative: f(2*x) = 2*f(x)"));
          facts_sub->AppendSeparator();
          facts_sub->Append(
			    Event::popid_property_noun, _("noun: Not evaluated by default"),
			    _("f(x) stays f(x) even if the value of f(x) is computable"));
          facts_sub->Append(Event::popid_property_evfun,
                            _("ev() shall evaluate this automatically"),
                            _("Make an eventual ev() evaluate this"));
          facts_sub->Append(Event::popid_property_evflag,
        ...
        if ((selectionStyle == TS_CODE_VARIABLE) ||
          ...
          if (facts_sub->GetMenuItemCount() > 0)
            facts_sub->AppendSeparator();
          facts_sub->Append(Event::popid_property_alphabetic,
...
void Worksheet::OpenQuestionCaret(const wxString &txt) {
  ...
  if (!m_cellPointers.m_answerCell) {
	...
	new wxMenuEvent(wxEVT_MENU, Event::menu_evaluate);
...
void Worksheet::OnKeyDown(wxKeyEvent &event) {
  ...
  case WXK_NUMPAD_DELETE:
    if (event.ShiftDown()) {
      ...
      wxCommandEvent ev(wxEVT_COMMAND_MENU_SELECTED, Event::popid_delete);
      GetParent()->ProcessWindowEvent(ev);
    } else
      event.Skip();
    break;
  ...
  case WXK_RETURN:
    ...
    if (event.ControlDown() && event.ShiftDown()) {
      ...
      evaluateEvent->SetId(Event::Event::popid_evaluate_section);
      GetParent()->GetEventHandler()->QueueEvent(evaluateEvent);
    } else {
      ...
      if (enterEvaluates !=
          controlOrShift) { // shift-enter pressed === Event::menu_evaluate event
        Evaluate();
      } else
        event.Skip();
    }
        ...
        return false;
      {
        ...
        */
...

        {
          ...
          // In wxWidgets 3.1.1 fsystem->FindFirst crashes if we don't have a
          // file in the memory filesystem => Let's create a file just to make
          // sure one exists.
          wxMemoryBuffer dummyBuf;
          wxMemoryFSHandler::AddFile("dummyfile", dummyBuf.GetData(),
                                     dummyBuf.GetDataLen());
          ...
          while (memFsName != wxEmptyString) {
            ...
            if (name != wxT("dummyfile")) {
              ...
              if (fsfile) {
                // The data for gnuplot is likely to change in its entirety if
                // it ever changes => We can store it in a compressed form.
