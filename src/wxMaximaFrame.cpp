...
wxMaximaFrame::wxMaximaFrame(wxWindow *parent, int id, wxLocale *locale,
  ...
  entries[0].Set(wxACCEL_CTRL, wxT('K'), Event::menu_autocomplete);
  entries[1].Set(wxACCEL_CTRL, WXK_TAB, Event::menu_autocomplete);
  entries[2].Set(wxACCEL_CTRL | wxACCEL_SHIFT, WXK_TAB,
                 Event::menu_autocomplete_templates);
  entries[3].Set(wxACCEL_CTRL, WXK_SPACE, Event::menu_autocomplete);
  ...
  m_manager.GetPane(wxT("history")) = m_manager.GetPane(wxT("history"))
    ...
    .Movable(true);
  m_manager.Update();
  Connect(Event::menu_pane_dockAll, wxEVT_MENU,
...
void wxMaximaFrame::UpdateStatusMaximaBusy() {
  ...
  if ((m_StatusMaximaBusy != m_StatusMaximaBusy_next) ||
    ...
    if (!m_StatusSaving) {
      ...
      case userinput:
        m_bytesFromMaxima_last = 0;
        m_MenuBar->EnableItem(Event::menu_remove_output, false);
        RightStatusText(_("Maxima asks a question"));
        break;
      case sending:
        m_bytesFromMaxima_last = 0;
        m_MenuBar->EnableItem(Event::menu_remove_output, true);
...
#define APPEND_MENU_ITEM(menu, id, label, help, stock)	\
  ...
  m_FileMenu->Append(Event::menu_recent_documents, _("Open Recent"),
  ...
  m_FileMenu->Append(Event::menu_load_id, _("&Load Package...\tCtrl+L"),
                     _("Load a Maxima package file"), wxITEM_NORMAL);
  m_recentPackagesMenu = new wxMenu();
  m_FileMenu->Append(Event::menu_recent_packages, _("Load Recent Package"),
                     m_recentPackagesMenu);
  m_FileMenu->Append(Event::menu_batch_id, _("&Batch File...\tCtrl+B"),
                     _("Load a Maxima file using the batch command"),
                     wxITEM_NORMAL);
  m_FileMenu->Append(Event::menu_export_html, _("&Export..."),
  ...
  m_EditMenu->Append(Event::menu_copy_text_from_worksheet,
                     _("Copy as Text\tCtrl+Shift+C"),
                     _("Copy selection from document as text"), wxITEM_NORMAL);
  m_EditMenu->Append(
		     Event::menu_copy_matlab_from_worksheet, _("Copy for Octave/Matlab"),
		     _("Copy selection from document in Matlab format"), wxITEM_NORMAL);
  m_EditMenu->Append(Event::menu_copy_tex_from_worksheet, _("Copy as LaTeX"),
                     _("Copy selection from document in LaTeX format"),
                     wxITEM_NORMAL);
  m_EditMenu->Append(Worksheet::Event::popid_copy_mathml, _("Copy as MathML"),
                     _("Copy selection from document in a MathML format many "
                       "word processors can display as 2d equation"),
                     wxITEM_NORMAL);
  m_EditMenu->Append(Event::menu_copy_as_bitmap, _("Copy as Image"),
                     _("Copy selection from document as an image"),
                     wxITEM_NORMAL);
  m_EditMenu->Append(Event::menu_copy_as_svg, _("Copy as SVG"),
                     _("Copy selection from document as an SVG image"),
                     wxITEM_NORMAL);
  m_EditMenu->Append(Event::menu_copy_as_rtf, _("Copy as RTF"),
                     _("Copy selection from document as rtf that a word "
                       "processor might understand"),
                     wxITEM_NORMAL);
#if wxUSE_ENH_METAFILE
  m_EditMenu->Append(Event::menu_copy_as_emf, _("Copy as EMF"),
  ...
  m_EditMenu->Append(wxID_SELECTALL, _("Select All\tCtrl+A"), _("Select all"),
                     wxITEM_NORMAL);
  m_EditMenu->Append(Event::menu_copy_to_file, _("Save Selection to Image..."),
                     _("Save selection from document to an image file"),
                     wxITEM_NORMAL);
  m_EditMenu->AppendSeparator();
  m_EditMenu->Append(
		     Worksheet::Event::popid_comment_selection, _("Comment selection\tCtrl+/"),
  ...
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_show_toolbar,
                                      _("Main Toolbar\tAlt+Shift+B"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_math,
                                      _("General Math\tAlt+Shift+M"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_stats,
                                      _("Statistics\tAlt+Shift+S"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_greek,
                                      _("Greek Letters\tAlt+Shift+G"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_symbols,
                                      _("Symbols\tAlt+Shift+Y"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_unicode,
                                      _("Unicode chars\tAlt+Shift+U"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_history,
                                      _("History\tAlt+Shift+I"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_structure,
                                      _("Table of Contents\tAlt+Shift+T"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_format,
                                      _("Insert Cell\tAlt+Shift+C"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_draw, _("Plot using Draw"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_help,
                                      _("The integrated help browser"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_log, _("Debug messages"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_variables, _("Variables"));
  m_Maxima_Panes_Sub->AppendCheckItem(Event::menu_pane_xmlInspector,
                                      _("Raw XML monitor"));
  m_Maxima_Panes_Sub->AppendSeparator();
  m_Maxima_Panes_Sub->Append(Event::menu_pane_dockAll, _("Dock all Sidebars"));
  m_Maxima_Panes_Sub->AppendSeparator();
  m_Maxima_Panes_Sub->AppendCheckItem(ToolBar::tb_hideCode,
                                      _("Hide Code Cells\tAlt+Ctrl+H"));
  m_Maxima_Panes_Sub->Append(Event::menu_pane_hideall,
  ...
  m_equationTypeMenuMenu->AppendRadioItem(
					  Event::menu_math_as_1D_ASCII, _("as 1D ASCII"),
					  _("Show equations in their linear form"));
  m_equationTypeMenuMenu->AppendRadioItem(Event::menu_math_as_2D_ASCII,
                                          _("as ASCII Art"),
                                          _("2D equations using ASCII Art"));
  m_equationTypeMenuMenu->AppendRadioItem(Event::menu_math_as_graphics, _("in 2D"),
  ...
  m_autoSubscriptMenu->AppendRadioItem(
				       Event::menu_alwaysAutosubscript, _("Always after underscores"),
				       _("Always autosubscript after an underscore"));
  m_autoSubscriptMenu->AppendRadioItem(
				       Event::menu_defaultAutosubscript, _("Only Integers and single letters"),
				       _("Autosubscript numbers and text following single letters"));
  m_autoSubscriptMenu->AppendRadioItem(
				       Event::menu_noAutosubscript, _("Never"),
  ...
  m_autoSubscriptMenu->Append(Event::menu_autosubscriptIndividual,
                              _("Always display this variable with subscript"));
  m_autoSubscriptMenu->Append(Event::menu_noAutosubscriptIndividual,
                              _("Never display this variable with subscript"));
  m_autoSubscriptMenu->Append(
			      Event::menu_declareAutosubscript,
  ...
  m_roundedMatrixParensMenu->AppendRadioItem(
					     Event::menu_roundedMatrixParens, _("Rounded"),
					     _("Use rounded parenthesis for matrices"));
  m_roundedMatrixParensMenu->AppendRadioItem(
					     Event::menu_squareMatrixParens, _("Square"),
					     _("Use square parenthesis for matrices"));
  m_roundedMatrixParensMenu->AppendRadioItem(
					     Event::menu_angledMatrixParens, _("Angles"),
					     _("Use \"<\" and \">\" as parenthesis for matrices"));
  m_roundedMatrixParensMenu->AppendRadioItem(
					     Event::menu_straightMatrixParens, _("Straight lines"),
					     _("Use vertical lines instead of parenthesis for matrices"));
  m_roundedMatrixParensMenu->AppendRadioItem(
					     Event::menu_noMatrixParens, _("None"), _("Don't use parenthesis for matrices"));
  m_viewMenu->Append(wxNewId(), _("Matrix parenthesis"),
                     m_roundedMatrixParensMenu,
                     _("Choose the parenthesis type for Matrices"));
  m_viewMenu->AppendCheckItem(Event::menu_stringdisp,
  ...
  m_Edit_Zoom_Sub->Append(Event::menu_zoom_80, wxT("80%"), _("Set zoom to 80%"),
                          wxITEM_NORMAL);
  m_Edit_Zoom_Sub->Append(wxID_ZOOM_100, wxT("100%"), _("Set zoom to 100%"),
                          wxITEM_NORMAL);
  m_Edit_Zoom_Sub->Append(Event::menu_zoom_120, wxT("120%"), _("Set zoom to 120%"),
                          wxITEM_NORMAL);
  m_Edit_Zoom_Sub->Append(Event::menu_zoom_150, wxT("150%"), _("Set zoom to 150%"),
                          wxITEM_NORMAL);
  m_Edit_Zoom_Sub->Append(Event::menu_zoom_200, wxT("200%"), _("Set zoom to 200%"),
                          wxITEM_NORMAL);
  m_Edit_Zoom_Sub->Append(Event::menu_zoom_300, wxT("300%"), _("Set zoom to 300%"),
                          wxITEM_NORMAL);

  m_viewMenu->Append(wxNewId(), _("Set Zoom"), m_Edit_Zoom_Sub, _("Set Zoom"));
#ifdef __UNIX__
  m_viewMenu->Append(Event::menu_fullscreen, _("Full Screen\tF11"),
                     _("Toggle full screen editing"), wxITEM_NORMAL);
#else
  m_viewMenu->Append(Event::menu_fullscreen, _("Full Screen\tAlt-Enter"),
                     _("Toggle full screen editing"), wxITEM_NORMAL);
#endif
  m_viewMenu->AppendSeparator();
  m_viewMenu->AppendCheckItem(Event::menu_invertWorksheetBackground,
  ...
  {
    wxMenuItem *it =
      new wxMenuItem(m_CellMenu, Event::menu_evaluate, _("Evaluate Cell(s)"),
  ...
  m_CellMenu->Append(
		     Event::Event::menu_evaluate_all_visible, _("Evaluate All Visible Cells\tCtrl+R"),
		     _("Evaluate all visible cells in the document"), wxITEM_NORMAL);
  {
    wxMenuItem *it = new wxMenuItem(
				    m_CellMenu, Event::menu_evaluate_all, _("Evaluate All Cells\tCtrl+Shift+R"),
  ...
  m_CellMenu->Append(Event::menu_remove_output, _("Remove All Output"),
                     _("Remove output from input cells"), wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  m_CellMenu->Append(Event::menu_insert_previous_input,
                     _("Copy Previous Input\tCtrl+I"),
                     _("Create a new cell with previous input"), wxITEM_NORMAL);
  m_CellMenu->Append(
		     Event::menu_insert_previous_output, _("Copy Previous Output\tCtrl+U"),
		     _("Create a new cell with previous output"), wxITEM_NORMAL);
  m_CellMenu->Append(Event::menu_autocomplete, _("Complete Word\tCtrl+Space"),
                     _("Complete word"), wxITEM_NORMAL);
  m_CellMenu->Append(Event::menu_autocomplete_templates,
                     _("Show Template\tCtrl+Shift+Space"),
                     _("Show function template"), wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  wxMenu *insert_sub = new wxMenu;
  insert_sub->Append(Event::menu_insert_input, _("Insert Input &Cell\tCtrl+0"),
                     _("Insert a new input cell"));
  insert_sub->Append(Event::menu_add_comment, _("Insert &Text Cell\tCtrl+1"),
                     _("Insert a new text cell"));
  insert_sub->Append(Event::menu_add_title, _("Insert T&itle Cell\tCtrl+2"),
                     _("Insert a new title cell"));
  insert_sub->Append(Event::menu_add_section, _("Insert &Section Cell\tCtrl+3"),
                     _("Insert a new section cell"));
  insert_sub->Append(Event::menu_add_subsection, _("Insert S&ubsection Cell\tCtrl+4"),
                     _("Insert a new subsection cell"));
  insert_sub->Append(Event::menu_add_subsubsection,
                     _("Insert S&ubsubsection Cell\tCtrl+5"),
                     _("Insert a new subsubsection cell"));
  insert_sub->Append(Event::menu_add_heading5, _("Insert heading5 Cell\tCtrl+6"),
                     _("Insert a new heading5 cell"));
  insert_sub->Append(Event::menu_add_heading6, _("Insert heading6 Cell\tCtrl+7"),
                     _("Insert a new heading6 cell"));
  insert_sub->Append(Event::menu_add_pagebreak, _("Insert Page Break"),
                     _("Insert a page break"));
  m_CellMenu->Append(wxNewId(), _("Insert textbased cell"), insert_sub);
  m_CellMenu->Append(Event::menu_insert_image, _("Insert Image..."), _("Insert image"),
                     wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  m_CellMenu->Append(Event::menu_fold_all_cells, _("Fold All\tCtrl+Alt+["),
                     _("Fold all sections"), wxITEM_NORMAL);
  m_CellMenu->Append(Event::menu_unfold_all_cells, _("Unfold All\tCtrl+Alt+]"),
                     _("Unfold all folded sections"), wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  m_CellMenu->Append(Event::menu_history_previous, _("Previous Command\tAlt+Up"),
                     _("Recall previous command from history"), wxITEM_NORMAL);
  m_CellMenu->Append(Event::menu_history_next, _("Next Command\tAlt+Down"),
                     _("Recall next command from history"), wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  m_CellMenu->Append(Worksheet::Event::popid_merge_cells, _("Merge Cells\tCtrl+M"),
                     _("Merge the text from two input cells into one"),
                     wxITEM_NORMAL);
  m_CellMenu->Append(Worksheet::Event::popid_divide_cell, _("Divide Cell\tCtrl+D"),
                     _("Divide this input cell into two cells"), wxITEM_NORMAL);
  m_CellMenu->AppendSeparator();
  m_CellMenu->AppendCheckItem(
			      Worksheet::Event::popid_auto_answer, _("Automatically answer questions"),
  ...
  {
    wxMenuItem *it =
      new wxMenuItem(m_MaximaMenu, Event::menu_interrupt_id, _("&Interrupt\tCtrl+G"),
  ...
  m_MaximaMenu->Append(Event::menu_soft_restart, _("&Clear Memory"),
                       _("Delete all values from memory"), wxITEM_NORMAL);
  APPEND_MENU_ITEM(m_MaximaMenu, Event::menu_add_path, _("Add to &Path..."),
                   _("Add a directory to search path"), wxT("gtk-add"));

  m_MaximaMenu->AppendSeparator();
  wxMenu *infolists_sub = new wxMenu;
  infolists_sub->Append(Event::menu_functions, _("&Functions"),
                        _("Show defined functions"), wxITEM_NORMAL);
  infolists_sub->Append(Event::menu_variables, _("&Variables"),
                        _("Show defined variables"), wxITEM_NORMAL);
  infolists_sub->Append(Event::menu_arrays, _("Arrays"));
  infolists_sub->Append(Event::menu_macros, _("Macros"));
  infolists_sub->Append(Event::menu_labels, _("Labels"));
  infolists_sub->Append(Event::menu_myoptions, _("Changed option variables"));
  infolists_sub->Append(Event::menu_rules, _("Rules"));
  infolists_sub->Append(Event::menu_aliases, _("Aliases"));
  infolists_sub->Append(Event::menu_structs, _("Structs"));
  infolists_sub->Append(Event::menu_dependencies, _("Dependencies"));
  infolists_sub->Append(Event::menu_gradefs, _("Gradefs"));
  infolists_sub->Append(Event::menu_let_rule_packages, _("Letrules"));
  m_MaximaMenu->Append(wxNewId(), _("Show user definitions"), infolists_sub);

  m_MaximaMenu->Append(Event::menu_clear_fun, _("Delete F&unction..."),
                       _("Delete a function"), wxITEM_NORMAL);
  m_MaximaMenu->Append(Event::menu_clear_var, _("Delete V&ariable..."),
                       _("Delete a variable"), wxITEM_NORMAL);
  m_MaximaMenu->Append(Event::menu_kill, _("Delete named object..."),
                       _("Delete all objects with a given name"),
                       wxITEM_NORMAL);

  m_MaximaMenu->AppendSeparator();
  m_MaximaMenu->AppendCheckItem(Event::menu_time, _("&Time Display"),
                                _("Display time used for evaluation"));
  m_MaximaMenu->Append(
		       Event::menu_display, _("Change &2d Display"),
		       _("Change the 2d display algorithm used to display math output"),
		       wxITEM_NORMAL);
  m_MaximaMenu->Append(Event::menu_texform, _("Display Te&X Form"),
                       _("Display last result in TeX form"), wxITEM_NORMAL);
  m_MaximaMenu->Append(Event::menu_grind, _("Maxima input"),
                       _("Convert a command to maxima code"), wxITEM_NORMAL);

  m_MaximaMenu->AppendSeparator();
  m_MaximaMenu->Append(
		       Event::menu_jumptoerror, _("Jump to first error"),
		       _("Jump to the first cell Maxima has reported an error in."),
		       wxITEM_NORMAL);
  // debugger type submenu
  m_debugTypeMenu = new wxMenu;
  m_debugTypeMenu->AppendRadioItem(Event::menu_debugmode_off, _("Never"));
  m_debugTypeMenu->AppendRadioItem(Event::menu_debugmode_lisp, _("On lisp errors"));
  m_debugTypeMenu->AppendRadioItem(Event::menu_debugmode_all, _("On all errors"));
  m_debugTypeMenu->Check(Event::menu_debugmode_off, true);
  m_MaximaMenu->Append(Event::menu_debugmode, _("Debugger trigger"), m_debugTypeMenu,
                       _("When to invoke the lisp compiler's debugger"));
  m_MaximaMenu->Enable(Event::menu_debugmode, false);

  wxMenu *programMenu = new wxMenu;
  programMenu->Append(Event::menu_for, _("For loop..."));
  programMenu->Append(Event::menu_while, _("While loop..."));
  programMenu->Append(Event::menu_list_do_for_each_element, _("do for each element"));
  programMenu->AppendSeparator();
  programMenu->Append(Event::menu_block, _("Program block with local variables..."));
  programMenu->Append(Event::menu_block_noLocal,
                      _("Program block, no local variables..."));
  programMenu->Append(Event::menu_return, _("Return from current block/loop..."));
  programMenu->Append(Event::menu_local, _("Make function local..."));
  programMenu->AppendSeparator();
  programMenu->Append(Event::menu_def_fun, _("Define function..."));
  m_MaximaMenu->Append(Event::menu_fun_def, _("Show function &Definition..."),
                       _("Show definition of a function"), wxITEM_NORMAL);
  programMenu->Append(Event::menu_lambda, _("unnamed function (lambda)..."));
  programMenu->Append(Event::menu_quotequote, _("Compile function..."));
  programMenu->Append(Event::menu_paramType, _("Define parameter type..."));
  programMenu->Append(Event::menu_trace, _("Trace a function..."));
  programMenu->Append(Event::menu_def_macro, _("Define macro..."));
  programMenu->Append(Event::menu_def_variable, _("Define variable..."));
  programMenu->Append(Event::menu_structdef, _("Define struct..."));
  programMenu->Append(Event::menu_structnew, _("Create struct..."));
  programMenu->Append(Event::menu_structuse, _("Use struct field..."));
  programMenu->AppendSeparator();
  programMenu->Append(Event::menu_quote, _("Variable name, not contents..."));
  programMenu->Append(Event::menu_quoteblock, _("Don't evaluate Expression..."));
  programMenu->Append(Event::menu_quotequote, _("Interpret like input..."));
  programMenu->Append(Event::menu_maximatostring, _("Expression to string..."));
  programMenu->Append(Event::menu_stringtomaxima, _("Interpret string..."));
  programMenu->AppendSeparator();
  programMenu->Append(Event::menu_saveLisp, _("Save as lisp..."));
  programMenu->Append(Event::menu_loadLisp, _("Load lisp..."));
  programMenu->AppendSeparator();
  programMenu->Append(Event::menu_gensym, _("Generate unused symbol name"));
  m_MaximaMenu->Append(wxNewId(), _("Program"), programMenu);
  wxMenu *stringMenu = new wxMenu;
  wxMenu *streamMenu = new wxMenu;
  streamMenu->Append(Event::menu_stringproc_openr, _("Open for reading..."));
  streamMenu->Append(Event::menu_stringproc_opena, _("Open for appending..."));
  streamMenu->Append(Event::menu_stringproc_openw, _("Open for writing..."));
  streamMenu->Append(Event::menu_stringproc_setposition, _("Set position..."));
  streamMenu->Append(Event::menu_stringproc_getposition, _("Get position..."));
  streamMenu->Append(Event::menu_stringproc_flush_output, _("Flush..."));
  streamMenu->Append(Event::menu_stringproc_flength, _("Length..."));
  streamMenu->Append(Event::menu_stringproc_close, _("Close..."));
  stringMenu->Append(wxNewId(), _("Stream"), streamMenu);
  wxMenu *ioMenu = new wxMenu;
  ioMenu->Append(Event::menu_stringproc_printf, _("printf..."));
  ioMenu->Append(Event::menu_stringproc_readline, _("readline..."));
  ioMenu->Append(Event::menu_stringproc_readchar, _("readchar..."));
  ioMenu->Append(Event::menu_stringproc_readbyte, _("readbyte..."));
  ioMenu->Append(Event::menu_stringproc_writebyte, _("writebyte..."));
  stringMenu->Append(wxNewId(), _("I/O"), ioMenu);
  wxMenu *charTestMenu = new wxMenu;
  charTestMenu->Append(Event::menu_stringproc_charp, _("Is a char?..."));
  charTestMenu->Append(Event::menu_stringproc_alphacharp, _("Is alphabetic?..."));
  charTestMenu->Append(Event::menu_stringproc_alphanumericp, _("Is alphanumeric?..."));
  charTestMenu->Append(Event::menu_stringproc_digitcharp, _("Is a digit?..."));
  charTestMenu->Append(Event::menu_stringproc_constituent, _("Is printable?..."));
  charTestMenu->Append(Event::menu_stringproc_uppercasep, _("Is uppercase?..."));
  charTestMenu->Append(Event::menu_stringproc_lowercasep, _("Is lowercase?..."));
  charTestMenu->Append(Event::menu_stringproc_cequal, _("Is equal?..."));
  charTestMenu->Append(Event::menu_stringproc_cequalignore,
                       _("Equal, ignoring case?..."));
  charTestMenu->Append(Event::menu_stringproc_clessp, _("Is lower?..."));
  charTestMenu->Append(Event::menu_stringproc_clesspignore,
                       _("Lower, ignoring case?..."));
  charTestMenu->Append(Event::menu_stringproc_cgreaterp, _("Is greater?..."));
  charTestMenu->Append(Event::menu_stringproc_cgreaterpignore,
                       _("Greater, ignoring case?..."));
  stringMenu->Append(wxNewId(), _("Character Tests"), charTestMenu);
  wxMenu *stringtestMenu = new wxMenu;
  stringtestMenu->Append(Event::menu_stringproc_sequal, _("Equal?..."));
  stringtestMenu->Append(Event::menu_stringproc_sequalignore,
                         _("Equal, ignoring case?..."));
  stringMenu->Append(wxNewId(), _("String Tests"), stringtestMenu);
  wxMenu *convertMenu = new wxMenu;
  convertMenu->Append(Event::menu_stringproc_create_ascii, _("Ascii code to char..."));
  convertMenu->Append(Event::menu_stringproc_ascii, _("char to Ascii code..."));
  convertMenu->Append(Event::menu_stringproc_cint, _("Char to Unicode code point..."));
  convertMenu->Append(Event::menu_stringproc_unicode,
                      _("Unicode code point to char..."));
  convertMenu->Append(Event::menu_stringproc_unicode_to_utf8,
                      _("Unicode code point to UTF8..."));
  convertMenu->Append(Event::menu_stringproc_utf8_to_unicode,
                      _("UTF8 to Unicode code point..."));
  convertMenu->Append(Event::menu_stringproc_charlist,
                      _("String to list of chars..."));
  convertMenu->Append(Event::menu_stringproc_simplode,
                      _("List of chars to string..."));
  convertMenu->Append(Event::menu_stringproc_eval_string, _("Evaluate string..."));
  convertMenu->Append(Event::menu_stringproc_parse_string, _("Parse string only..."));
  convertMenu->Append(Event::menu_stringproc_number_to_octets,
                      _("Number to octets..."));
  convertMenu->Append(Event::menu_stringproc_octets_to_number,
                      _("Octets to number..."));
  convertMenu->Append(Event::menu_stringproc_octets_to_string,
                      _("Octets to string..."));
  convertMenu->Append(Event::menu_stringproc_string_to_octets,
                      _("String to octets..."));
  stringMenu->Append(wxNewId(), _("Conversions"), convertMenu);
  wxMenu *transformMenu = new wxMenu;
  transformMenu->Append(Event::menu_stringproc_charat, _("Extract char..."));
  transformMenu->Append(Event::menu_stringproc_sinsert, _("Insert char..."));
  transformMenu->Append(Event::menu_stringproc_scopy, _("Copy string..."));
  transformMenu->Append(Event::menu_stringproc_sdowncase, _("Convert to downcase..."));
  transformMenu->Append(Event::menu_stringproc_slength, _("Length..."));
  transformMenu->Append(Event::menu_stringproc_smake, _("Create empty string..."));
  transformMenu->Append(Event::menu_stringproc_smismatch,
                        _("Find first difference..."));
  transformMenu->Append(Event::menu_stringproc_split, _("Split..."));
  transformMenu->Append(Event::menu_stringproc_sposition, _("Find char in string..."));
  transformMenu->Append(Event::menu_stringproc_sremove,
                        _("Remove all occurrences of a word..."));
  transformMenu->Append(Event::menu_stringproc_sremovefirst,
                        _("Remove first occurrence of a word..."));
  transformMenu->Append(Event::menu_stringproc_tokens, _("Tokenize..."));
  transformMenu->Append(Event::menu_stringproc_ssearch, _("Search..."));
  transformMenu->Append(Event::menu_stringproc_ssort, _("Sort the characters..."));
  transformMenu->Append(Event::menu_stringproc_ssubstfirst, _("Replace..."));
  transformMenu->Append(Event::menu_stringproc_strim, _("Trim both ends..."));
  transformMenu->Append(Event::menu_stringproc_striml, _("Trim left..."));
  transformMenu->Append(Event::menu_stringproc_strimr, _("Trim right..."));
  ...
  regexMenu->Append(Event::menu_sregex_load, _("Load the regex processor"));
  regexMenu->Append(Event::menu_sregex_regex_compile, _("Compile a regex"));
  regexMenu->Append(Event::menu_sregex_regex_match_pos, _("Position of a match"));
  regexMenu->Append(Event::menu_sregex_regex_match, _("Return a match"));
  regexMenu->Append(Event::menu_sregex_regex_split, _("Split on match"));
  regexMenu->Append(Event::menu_sregex_subst_first, _("Substitute first match"));
  regexMenu->Append(Event::menu_sregex_regex_subst, _("Substitute all matches"));
  regexMenu->Append(Event::menu_sregex_string_to_regex,
                    _("Regex that matches a string"));
  stringMenu->Append(wxNewId(), _("RegEx"), regexMenu);
  wxMenu *operatingSystemMenu = new wxMenu;
  regexMenu->Append(Event::menu_opsyst_load, _("Load the file/dir operations"));
  wxMenu *dirMenu = new wxMenu;
  dirMenu->Append(Event::menu_opsyst_directory, _("List directory"));
  dirMenu->Append(Event::menu_opsyst_getcurrentdirectory, _("Get current directory"));
  dirMenu->Append(Event::menu_opsyst_chdir, _("Change directory..."));
  dirMenu->Append(Event::menu_opsyst_mkdir, _("Create directory..."));
  dirMenu->Append(Event::menu_opsyst_rmdir, _("Remove directory..."));
  dirMenu->Append(Event::menu_opsyst_directory, _("List directory..."));
  dirMenu->Append(Event::menu_opsyst_pathname_directory,
                  _("Extract dir from path..."));
  dirMenu->Append(Event::menu_opsyst_pathname_name,
                  _("Extract filename from path..."));
  dirMenu->Append(Event::menu_opsyst_pathname_type,
                  _("Extract filetype from path..."));
  operatingSystemMenu->Append(wxNewId(), _("Directory operations"), dirMenu);
  wxMenu *fileMenu = new wxMenu;
  fileMenu->Append(Event::menu_opsyst_copy_file, _("Copy file..."));
  fileMenu->Append(Event::menu_opsyst_rename_file, _("Rename file..."));
  fileMenu->Append(Event::menu_opsyst_delete_file, _("Delete file..."));
  operatingSystemMenu->Append(wxNewId(), _("File operations"), fileMenu);
  wxMenu *envMenu = new wxMenu;
  envMenu->Append(Event::menu_opsyst_getenv, _("Read environment variable..."));
  ...
  m_gentranMenu->Append(Event::gentran_load, _("Load the translation generator"));
  m_gentranMenu->AppendRadioItem(Event::gentran_lang_c, _("Output C"));
  m_gentranMenu->AppendRadioItem(Event::gentran_lang_fortran, _("Output Fortran"));
  m_gentranMenu->AppendRadioItem(Event::gentran_lang_ratfor,
                                 _("Output Rational Fortran"));
  m_gentranMenu->Append(Event::gentran_to_stdout, _("Convert"));
  m_gentranMenu->Append(Event::gentran_to_file, _("Convert + Write to file"));
  ...
  solve_sub->Append(Event::menu_solve, _("&Solve..."), _("Solve equation(s)"),
                    wxITEM_NORMAL);
  solve_sub->Append(Event::menu_solve_to_poly, _("Solve (to_poly)..."),
                    _("Solve equation(s) with to_poly_solve"), wxITEM_NORMAL);
  solve_sub->Append(Event::menu_solve_lin, _("Solve &Linear System..."),
                    _("Solve linear system of equations"), wxITEM_NORMAL);
  solve_sub->Append(Event::menu_solve_algsys, _("Solve &Algebraic System..."),
                    _("Solve algebraic system of equations"), wxITEM_NORMAL);
  solve_sub->Append(Event::menu_eliminate, _("&Eliminate Variable..."),
  ...
  solveNum1_sub->Append(Event::menu_solve_num, _("Find numerical solution..."),
                        _("Find a root of an equation on an interval"),
                        wxITEM_NORMAL);
  solveNum1_sub->Append(
			Event::menu_realroots, _("Dito, but as fraction (real only)..."),
			_("Find fractions that real roots of a polynomial and "), wxITEM_NORMAL);
  solveNum1_sub->Append(Event::menu_allroots,
                        _("Numerical solutions of polynomial..."),
                        _("Find all roots of a polynomial"), wxITEM_NORMAL);
  solveNum1_sub->Append(
			Event::menu_bfallroots, _("Numerical solutions of polynomial..."),
  ...
  m_EquationsMenu->Append(Event::menu_solve_ode, _("Solve &ODE..."),
                          _("Solve ordinary differential equation "
                            "of maximum degree 2"),
                          wxITEM_NORMAL);
  m_EquationsMenu->Append(Event::menu_ivp_1, _("Initial Value Problem (&1)..."),
                          _("Solve initial value problem for first"
                            " degree ODE"),
                          wxITEM_NORMAL);
  m_EquationsMenu->Append(Event::menu_ivp_2, _("Initial Value Problem (&2)..."),
                          _("Solve initial value problem for second "
                            "degree ODE"),
                          wxITEM_NORMAL);
  m_EquationsMenu->Append(Event::menu_bvp, _("&Boundary Value Problem..."),
                          _("Solve boundary value problem for second "
                            "degree ODE"),
                          wxITEM_NORMAL);
  m_EquationsMenu->Append(Event::menu_rk, _("Numerical solution..."),
                          _("Find a numerical solution for a first "
                            "degree ODE"),
                          wxITEM_NORMAL);
  m_EquationsMenu->AppendSeparator();
  m_EquationsMenu->Append(Event::menu_solve_de, _("Solve ODE with Lapla&ce..."),
                          _("Solve ordinary differential equations "
                            "with Laplace transformation"),
                          wxITEM_NORMAL);
  m_EquationsMenu->Append(Event::menu_atvalue, _("A&t Value..."),
                          _("Setup atvalues for solving ODE with "
                            "Laplace transformation"),
                          wxITEM_NORMAL);
  m_EquationsMenu->AppendSeparator();
  m_EquationsMenu->Append(
			  Event::menu_lhs, _("Left side to the \"=\""),
			  _("The half of the equation that is to the left of the \"=\""),
			  wxITEM_NORMAL);
  m_EquationsMenu->Append(
			  Event::menu_rhs, _("Right side to the \"=\""),
			  _("The half of the equation that is to the right of the \"=\""),
			  wxITEM_NORMAL);
  m_EquationsMenu->AppendSeparator();
  m_EquationsMenu->Append(Event::menu_construct_fraction, _("Construct fraction..."));
  m_MenuBar->Append(m_EquationsMenu, _("E&quations"));

  // Matrix menu
  m_matrix_menu = new wxMenu;
  wxMenu *gen_matrix_menu = new wxMenu;
  gen_matrix_menu->Append(Event::menu_genmatrix, _("2D Array to Matrix..."),
                          _("Extract a matrix from a 2-dimensional array"),
                          wxITEM_NORMAL);
  gen_matrix_menu->Append(
			  Event::menu_gen_mat_lambda, _("Generate Matrix from Expression..."),
			  _("Generate a matrix from a lambda expression"), wxITEM_NORMAL);
  gen_matrix_menu->Append(Event::menu_enter_mat, _("&Enter Matrix..."),
                          _("Enter a matrix"), wxITEM_NORMAL);
  gen_matrix_menu->Append(Event::menu_list_list2matrix, _("Nested list to Matrix"),
                          _("Convert a list of lists to a matrix"),
                          wxITEM_NORMAL);
  gen_matrix_menu->Append(Event::menu_csv2mat, _("Matrix from csv file..."),
  ...
  fileio_menu->Append(Event::menu_csv2mat, _("Matrix from csv file"),
                      _("Load a matrix from a csv file"), wxITEM_NORMAL);
  fileio_menu->Append(Event::menu_mat2csv, _("Matrix to csv file"),
  ...
  matrix_basic_sub->Append(Event::menu_matrix_multiply, _("Multiply matrices..."));
  matrix_basic_sub->Append(Event::menu_matrix_exponent, _("Matrix exponent..."));
  matrix_basic_sub->Append(Event::menu_matrix_hadamard_product,
                           _("Hadamard (element-by-element) product..."),
                           _("Element-by-element multiplication"),
                           wxITEM_NORMAL);
  matrix_basic_sub->Append(
			   Event::menu_matrix_hadamard_exponent, _("Hadamard exponent..."),
			   _("Repetitive element-by-element multiplication"), wxITEM_NORMAL);
  matrix_basic_sub->Append(
			   Event::menu_copymatrix, _("Create copy, not clone..."),
  ...
  matrix_classicOP_menu->Append(Event::menu_invert_mat, _("&Invert Matrix"),
                                _("Compute the inverse of a matrix"),
                                wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_cpoly, _("&Characteristic Polynomial..."),
                                _("Compute the characteristic polynomial "
                                  "of a matrix"),
                                wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_determinant, _("&Determinant"),
                                _("Compute the determinant of a matrix"),
                                wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_eigen, _("Eigen&values"),
                                _("Find eigenvalues of a matrix"),
                                wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_eigvect, _("Eige&nvectors"),
                                _("Find eigenvectors of a matrix"),
                                wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_adjoint_mat, _("Ad&joint Matrix"),
                                _("Compute the adjoint matrix"), wxITEM_NORMAL);
  matrix_classicOP_menu->Append(
				Event::menu_rank, _("Rank"), _("Compute the rank of a matrix"), wxITEM_NORMAL);
  matrix_classicOP_menu->Append(Event::menu_transpose, _("&Transpose Matrix"),
  ...
  lapack_menu->Append(Event::menu_matrix_loadLapack, _("Load lapack"),
                      _("Load lapack"), wxITEM_NORMAL);
  lapack_menu->Append(Event::menu_matrix_dgeev_eigenvaluesOnly,
                      _("Eigenvalues (real)"),
                      _("Compile eigenvalues using dgeev"), wxITEM_NORMAL);
  lapack_menu->Append(Event::menu_matrix_zgeev_eigenvaluesOnly,
                      _("Eigenvalues (complex)"),
                      _("Compile eigenvalues using zgeev"), wxITEM_NORMAL);
  lapack_menu->Append(
		      Event::menu_matrix_dgeev, _("Eigenvalues, left+right eigenvectors (real)"),
		      _("Compile eigenvalues+eigenvectors using dgeev"), wxITEM_NORMAL);
  lapack_menu->Append(Event::menu_matrix_zgeev,
                      _("Eigenvalues, left+right eigenvectors (complex)"),
                      _("Compile eigenvalues+eigenvectors using zgeev"));
  lapack_menu->Append(Event::menu_matrix_dgeqrf, _("QR decomposition"),
                      _("Compile a QR decomposition using dgeqrf"));
  lapack_menu->Append(Event::menu_matrix_dgesv, _("Solve Ax=b"),
                      _("Solve a linear equation system using dgesv"));
  lapack_menu->Append(Event::Event::menu_matrix_dgesvd_valuesOnly, _("Singular Values"),
                      _("Singular values using dgesvd"));
  lapack_menu->Append(Event::menu_matrix_dgesvd,
                      _("Singular values, left + right vectors"),
                      _("Singular values using dgesvd"));
  lapack_menu->Append(Event::menu_matrix_dlange_max, _("max(abs(A(i,j))) (real)"));
  lapack_menu->Append(Event::menu_matrix_zlange_max, _("max(abs(A(i,j))) (complex)"));
  lapack_menu->Append(Event::menu_matrix_dlange_one, _("L[1] Norm (real)"));
  lapack_menu->Append(Event::menu_matrix_zlange_one, _("L[1] Norm (complex)"));
  lapack_menu->Append(Event::menu_matrix_dlange_inf, _("L[inf] Norm (real)"));
  lapack_menu->Append(Event::menu_matrix_zlange_inf, _("L[inf] Norm (complex)"));
  lapack_menu->Append(Event::menu_matrix_dlange_frobenius,
                      _("Frobenius Norm sqrt(sum((A(i,j))^2)) (real)"));
  lapack_menu->Append(Event::menu_matrix_zlange_frobenius,
                      _("Frobenius Norm sqrt(sum((A(i,j))^2)) (complex)"));
  // TODO: What is Event::menu_matrix_zheev (means: lapack's function zheev) for?
  ...
  matrix_rowOp_sub->Append(Event::menu_matrix_row, _("Extract Row..."),
                           _("Extract a row from the matrix"), wxITEM_NORMAL);
  matrix_rowOp_sub->Append(Event::menu_matrix_col, _("Extract Column..."),
                           _("Extract a column from the matrix"),
                           wxITEM_NORMAL);
  matrix_rowOp_sub->Append(Event::menu_submatrix, _("Remove Rows or Columns..."),
                           _("Remove rows and/or columns from the matrix"),
                           wxITEM_NORMAL);
  matrix_rowOp_sub->Append(
			   Event::menu_matrix_row_list, _("Convert Row to list..."),
			   _("Extract a row from the matrix and convert it to a list"),
			   wxITEM_NORMAL);
  matrix_rowOp_sub->Append(
			   Event::menu_matrix_col_list, _("Convert Column to list..."),
  ...
  m_matrix_menu->AppendSeparator();
  m_matrix_menu->Append(Event::menu_map_mat,
                        _("A&pply function to each Matrix element..."),
                        _("Map function to a matrix"), wxITEM_NORMAL);
  m_matrix_menu->Append(
			Event::menu_map, _("Dito, but affect all clones of the Matrix..."),
  ...
  m_CalculusMenu->Append(Event::menu_integrate, _("&Integrate..."),
                         _("Integrate expression"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_risch, _("Risch Integration..."),
                         _("Integrate expression with Risch algorithm"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_change_var, _("C&hange Variable in Integrate..."),
                         _("Change variable in integral or sum"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(
			 Event::menu_change_var_evaluate, _("Dito, and evaluate the result..."),
			 _("Change variable in integral or sum and evaluate the result"),
			 wxITEM_NORMAL);
  m_CalculusMenu->AppendSeparator();
  m_CalculusMenu->Append(Event::menu_diff, _("&Differentiate..."),
                         _("Differentiate expression"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_limit, _("Find &Limit..."),
                         _("Find a limit of an expression"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_lbfgs, _("Find Minimum..."),
                         _("Find a (unconstrained) minimum of an expression"),
                         wxITEM_NORMAL);
  wxMenu *series_sub = new wxMenu;
  series_sub->Append(Event::menu_taylor, _("Taylor series..."));
  series_sub->Append(Event::menu_powerseries, _("Power series..."));
  series_sub->Append(Event::menu_fourier, _("Fourier coefficients..."));
  series_sub->Append(Event::menu_pade, _(wxT("P&ad\u00E9 Approximation...")),
                     _("Pade approximation of a Taylor series"));
  m_CalculusMenu->Append(wxNewId(), _("Series approximation"), series_sub);
  m_CalculusMenu->Append(Event::menu_sum, _("Calculate Su&m..."), _("Calculate sums"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_product, _("Calculate &Product..."),
                         _("Calculate products"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_laplace, _("Laplace &Transform..."),
                         _("Get Laplace transformation of an expression"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(
			 Event::menu_ilt, _("Inverse Laplace T&ransform..."),
			 _("Get inverse Laplace transformation of an expression"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_gcd, _("&Greatest Common Divisor..."),
                         _("Compute the greatest common divisor"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_lcm, _("Least Common Multiple..."),
                         _("Compute the least common multiple "
                           "(do load(functs) before using)"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_divide, _("Di&vide Polynomials..."),
                         _("Divide numbers or polynomials"), wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_partfrac, _("Partial &Fractions..."),
                         _("Decompose rational function to partial fractions"),
                         wxITEM_NORMAL);
  m_CalculusMenu->Append(Event::menu_continued_fraction, _("&Continued Fraction"),
  ...
  simplify_sub->Append(Event::menu_mainvar, _("Set main variable..."));
  simplify_sub->Append(Event::menu_ratsimp, _("Try to guess which form is &simple"),
                       _("Simplify rational expression"), wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_radsimp, _("Simplify &Radicals..."),
                       _("Simplify expression containing radicals"),
                       wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_factor, _("&Factor Expression"),
                       _("Factor an expression"), wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_scanmapfactor,
                       _("Factor Expression including subexpressions"),
                       _("Factor an expression"), wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_gfactor, _("Factor Complex"),
                       _("Factor an expression in Gaussian numbers"),
                       wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_expand, _("&Expand Expression"),
                       _("Expand an expression"), wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_horner, _("Horner's rule"),
                       _("Reorganize an expression using horner's rule"),
                       wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_collapse, _("Optimize for memory"));
  simplify_sub->Append(Event::menu_optimize, _("Optimize for CPU time"));
  simplify_sub->Append(Event::menu_expandwrt, _("Expand for given variables"));
  simplify_sub->Append(Event::Event::menu_expandwrt_denom, _("Dito, including denominator"));
  simplify_sub->Append(Event::menu_scsimp, _("Sequential Comparative Simplification"));
  simplify_sub->Append(Event::menu_xthru, _("Find common denominator"));
  simplify_sub->Append(Event::menu_partfrac, _("Partial &Fractions..."),
                       _("Decompose rational function to partial fractions"),
                       wxITEM_NORMAL);
  simplify_sub->Append(Event::menu_simpsum, _("Simplify sum() commands..."));
  m_SimplifyMenu->Append(wxNewId(), _("Simplify equations"), simplify_sub);
  m_logexpand_Sub = new wxMenu;
  m_logexpand_Sub->Append(
			  Event::menu_logcontract, _("Contract Logarithms"),
			  _("Convert sum of logarithms to logarithm of product"), wxITEM_NORMAL);
  m_logexpand_Sub->Append(Event::menu_logexpand,
  ...
  m_logexpand_Sub->AppendRadioItem(Event::menu_logexpand_false, _("No"),
  ...
  m_Simplify_Gamma_Sub->Append(
			       Event::menu_to_fact, _("Convert to &Factorials"),
			       _("Convert binomials, beta and gamma function to factorials"),
			       wxITEM_NORMAL);
  m_Simplify_Gamma_Sub->Append(
			       Event::menu_to_gamma, _("Convert to &Gamma"),
			       _("Convert binomials, factorials and beta function to gamma function"),
			       wxITEM_NORMAL);
  m_Simplify_Gamma_Sub->Append(
			       Event::menu_factsimp, _("&Simplify Factorials"),
			       _("Simplify an expression containing factorials"), wxITEM_NORMAL);
  m_Simplify_Gamma_Sub->Append(Event::menu_factcomb, _("&Combine Factorials"),
  ...
  m_Simplify_Trig_Sub->Append(Event::menu_trigsimp, _("&Simplify Trigonometric"),
                              _("Simplify trigonometric expression"),
                              wxITEM_NORMAL);
  m_Simplify_Trig_Sub->Append(Event::menu_trigreduce, _("&Reduce Trigonometric"),
                              _("Reduce trigonometric expression"),
                              wxITEM_NORMAL);
  m_Simplify_Trig_Sub->Append(Event::menu_trigexpand, _("&Expand Trigonometric"),
                              _("Expand trigonometric expression"),
                              wxITEM_NORMAL);
  m_Simplify_Trig_Sub->Append(
			      Event::menu_trigrat, _("&Canonical Form"),
  ...
  m_Simplify_Complex_Sub->Append(Event::menu_rectform, _("Convert to &Rectform"),
                                 _("Convert complex expression to rect form"),
                                 wxITEM_NORMAL);
  m_Simplify_Complex_Sub->Append(Event::menu_polarform, _("Convert to &Polarform"),
                                 _("Convert complex expression to polar form"),
                                 wxITEM_NORMAL);
  m_Simplify_Complex_Sub->Append(Event::menu_realpart, _("Get Real P&art"),
                                 _("Get the real part of complex expression"),
                                 wxITEM_NORMAL);
  m_Simplify_Complex_Sub->Append(
				 Event::menu_imagpart, _("Get &Imaginary Part"),
				 _("Get the imaginary part of complex expression"), wxITEM_NORMAL);
  m_Simplify_Complex_Sub->Append(Event::menu_demoivre, _("&Demoivre"),
                                 _("Convert exponential function of imaginary "
                                   "argument to trigonometric form"),
                                 wxITEM_NORMAL);
  m_Simplify_Complex_Sub->Append(
				 Event::menu_exponentialize, _("&Exponentialize"),
  ...
  m_subst_Sub->Append(Event::menu_subst, _("Substitute..."),
                      _("A search-and-replace for equations"), wxITEM_NORMAL);
  m_subst_Sub->Append(Event::menu_ratsubst, _("Smart Substitute..."),
                      _("A subst with basic maths knowledge"), wxITEM_NORMAL);
  m_subst_Sub->Append(Event::menu_psubst, _("Parallel Substitute..."),
                      _("Substitutes, but not in the other substituents"),
                      wxITEM_NORMAL);
  m_subst_Sub->Append(Event::menu_fullratsubst, _("Recursive Substitute..."),
                      _("Substitutes until the equation no more changes"),
                      wxITEM_NORMAL);
  m_subst_Sub->Append(Event::menu_at, _("Subst constant t into eq with diff(x,t)..."),
                      _("Substitutes until the equation no more changes"),
                      wxITEM_NORMAL);
  m_subst_Sub->Append(Event::menu_subst, _("Substitute only in parts..."),
                      _("Substitute only in the elements n_1, n_2,..."),
                      wxITEM_NORMAL);
  m_subst_Sub->AppendCheckItem(Event::menu_opsubst,
                               _("Allow to substitute operators"));
  m_SimplifyMenu->Append(wxNewId(), _("Substitute"), m_subst_Sub);
  m_SimplifyMenu->Append(Event::menu_nouns, _("Evaluate &Noun Forms..."),
                         _("Evaluate all noun forms in expression"),
                         wxITEM_NORMAL);
  m_SimplifyMenu->AppendCheckItem(Event::menu_talg, _("&Algebraic Mode"),
                                  _("Set the \"algebraic\" flag"));
  m_SimplifyMenu->Append(Event::menu_tellrat, _("Add Algebraic E&quality..."),
                         _("Add equality to the rational simplifier"),
                         wxITEM_NORMAL);
  m_SimplifyMenu->Append(Event::menu_modulus, _("&Modulus Computation..."),
  ...
  listcreateSub->Append(
			Event::menu_list_create_from_elements, _("from individual elements"),
			_("Create a list from comma-separated elements"), wxITEM_NORMAL);
  listcreateSub->Append(Event::menu_list_create_from_rule, _("from a rule"),
                        _("Generate list elements using a rule"),
                        wxITEM_NORMAL);
  listcreateSub->Append(Event::menu_list_create_from_list, _("from a list"),
                        _("Generate a new list using a lists' elements"),
                        wxITEM_NORMAL);
  listcreateSub->Append(Event::menu_csv2list, _("Read List from csv file..."),
                        _("Load a list from a csv file"), wxITEM_NORMAL);
  listcreateSub->Append(Event::menu_list_actual_values_storage,
                        ...
                        wxITEM_NORMAL);
  listcreateSub->Append(
			Event::menu_list_create_from_args, _("from function arguments"),
  ...
  listuseSub->Append(Event::menu_list_map, _("apply function to each element"),
                     _("Runs each element through a function"), wxITEM_NORMAL);
  listuseSub->Append(
		     Event::menu_map_lambda, _("Run each element through an expression"),
		     _("Runs each element through an expression"), wxITEM_NORMAL);
  listuseSub->Append(
		     Event::menu_list_use_actual_values, _("use the actual values stored"),
		     _("Introduce the actual values for variables stored in the list"),
		     wxITEM_NORMAL);
  listuseSub->Append(
		     Event::menu_list_as_function_arguments, _("use as function arguments"),
		     _("Use list as the arguments of a function"), wxITEM_NORMAL);
  listuseSub->Append(Event::menu_list_do_for_each_element, _("do for each element"),
                     _("Execute a command for each element of the list"),
                     wxITEM_NORMAL);
  listuseSub->Append(Event::menu_list2csv, _("Export List to csv file..."),
  ...
  listextractmenu->Append(Event::menu_list_nth, _("nth"),
                          _("Returns an arbitrary list item"));
  listextractmenu->Append(Event::menu_list_first, _("First"),
                          _("Returns the first item of the list"));
  listextractmenu->Append(Event::menu_list_rest, _("All but the 1st n elements"),
                          _("Returns the list without its first n elements"));
  listextractmenu->Append(Event::menu_list_restN, _("All but the last n elements"),
                          _("Returns the list without its last n elements"));
  listextractmenu->Append(Event::menu_list_last, _("Last"),
                          _("Returns the last item of the list"));
  listextractmenu->Append(Event::menu_list_lastn, _("Last n"),
                          _("Returns the last n items of the list"));
  listextractmenu->Append(
			  Event::menu_list_extract_value, _("Extract an actual value for a variable"),
  ...
  listappendSub->Append(Event::menu_list_append_item_end, _("Append an element"),
                        _("Append an element to the end of an existing list"),
                        wxITEM_NORMAL);
  listappendSub->Append(
			Event::menu_list_append_item_start, _("Prepend an element"),
			_("Append an element to the beginning an existing list"), wxITEM_NORMAL);
  listappendSub->Append(Event::menu_list_append_list, _("Append a list"),
                        _("Append a list to an existing list"), wxITEM_NORMAL);
  listappendSub->Append(Event::menu_list_interleave, _("Interleave"),
                        _("Interleave the values of two lists"), wxITEM_NORMAL);
  m_listMenu->Append(wxNewId(), _("Append"), listappendSub, _("Use a list"));

  m_listMenu->Append(Event::menu_list_length, _("Length"),
                     _("Returns the length of the list"));
  m_listMenu->Append(Event::menu_list_reverse, _("Reverse"),
                     _("Reverse the order of the list items"));
  m_listMenu->AppendSeparator();
  m_listMenu->Append(Event::menu_list_sort, _("Sort"));
  m_listMenu->Append(Event::menu_list_remove_duplicates, _("Remove duplicates"),
                     _("Remove all list elements that appear twice in a row. "
                       "Normally used in conjunction with sort."));
  m_listMenu->AppendSeparator();
  m_listMenu->Append(Event::menu_list_push, _("Push"),
                     _("Add a new item to the beginning of the list. Useful "
                       "for creating stacks."));
  m_listMenu->Append(Event::menu_list_pop, _("Pop"),
                     _("Return the first item of the list and remove it from "
                       "the list. Useful for creating stacks."));
  m_listMenu->AppendSeparator();
  m_listMenu->Append(
		     Event::menu_list_list2matrix, _("Nested list to Matrix"),
		     _("Converts a nested list like [[1,2],[3,4]] to a matrix"));
  m_listMenu->Append(Event::menu_list_matrix2list, _("Matrix to nested List"),
                     _("Converts a matrix to a list of lists"));
  m_MenuBar->Append(m_listMenu, _("&List"));
  // Plot menu
  m_PlotMenu = new wxMenu;
  m_PlotMenu->Append(Event::gp_plot2, _("Plot &2d..."), _("Plot in 2 dimensions"),
                     wxITEM_NORMAL);
  m_PlotMenu->Append(Event::gp_plot3, _("Plot &3d..."), _("Plot in 3 dimensions"),
                     wxITEM_NORMAL);
  m_PlotMenu->Append(Event::menu_plot_format, _("Plot &Format..."),
                     _("Set plot format"), wxITEM_NORMAL);
  m_PlotMenu->AppendSeparator();
  m_PlotMenu->AppendCheckItem(Event::menu_animationautostart, _("Animation autoplay"),
                              _("Defines if an animation is automatically "
                                "started or only by clicking on it."));
  m_PlotMenu->Append(Event::menu_animationframerate, _("Animation framerate..."),
                     _("Set the frame rate for animations."));
  m_MenuBar->Append(m_PlotMenu, _("&Plot"));

  // Numeric menu
  m_NumericMenu = new wxMenu;
  m_NumericMenu->AppendCheckItem(Event::menu_num_out, _("&Numeric Output"),
                                 _("Numeric output"));
  m_NumericMenu->AppendCheckItem(
				 Event::menu_num_domain, _("Expect numbers harder to be complex"),
				 _("Expect variables to contain complex numbers"));
  m_NumericMenu->Check(Event::menu_num_domain, false);
  m_NumericMenu->Append(Event::menu_to_float, _("To &Float"),
                        _("Calculate float value of the last result"),
                        wxITEM_NORMAL);
  m_NumericMenu->Append(Event::menu_to_bfloat, _("To &Bigfloat"),
                        _("Calculate bigfloat value of the last result"),
                        wxITEM_NORMAL);
  m_NumericMenu->Append(Event::menu_to_numer, _("To Numeri&c\tCtrl+Shift+N"),
                        _("Calculate numeric value of the last result"),
                        wxITEM_NORMAL);

  wxMenu *floatToExactSub = new wxMenu;
  floatToExactSub->Append(
			  Event::menu_rationalize, _("To exact fraction"),
			  _("Find a fraction that represents this float exactly"), wxITEM_NORMAL);
  floatToExactSub->Append(
			  Event::menu_rat, _("Approximate by nice fraction"),
			  _("Find a nice fraction that is similar to this float"), wxITEM_NORMAL);
  floatToExactSub->Append(
			  Event::menu_guess_exact_value, _("Advanced guess (PSLQ algorithm)"),
  ...
  m_NumericMenu->Append(
			Event::menu_set_precision, _("Set bigfloat &Precision..."),
			...
			wxITEM_NORMAL);
  m_NumericMenu->Append(
			Event::menu_set_displayprecision, _("Set &displayed Precision..."),
			_("Shows how many digits of a numbers are displayed"), wxITEM_NORMAL);
  m_NumericMenu->AppendSeparator();
  m_NumericMenu->AppendCheckItem(
				 Event::menu_engineeringFormat, _("Engineering format (12.1e6 etc.)"),
				 _("Print floating-point numbers with exponents dividable by 3"));
  m_NumericMenu->Append(
			Event::menu_engineeringFormatSetup, _("Setup the engineering format..."),
  ...
  quadpack_sub->Append(Event::menu_quad_qag,
                       integralSign + _("(f(x),x,a,b), strategy of Aind"));
  quadpack_sub->Append(Event::menu_quad_qags,
                       integralSign + _("(f(x),x,a,b), Epsilon algorithm"));
  quadpack_sub->Append(Event::menu_quad_qagi,
                       integralSign + _("(f(x),x,a,b), infinite interval"));
  quadpack_sub->Append(Event::menu_quad_qawc,
                       _("Cauchy principal value, finite interval"));
  quadpack_sub->Append(Event::menu_quad_qawf_sin,
                       integralSign + wxT("(f(x)*sin(ω·x),x,a,∞)"));
  quadpack_sub->Append(Event::menu_quad_qawf_cos,
                       integralSign + wxT("(f(x)*cos(ω·x),x,a,∞)"));
  quadpack_sub->Append(Event::menu_quad_qawo_sin,
                       integralSign + wxT("(f(x)*sin(ω·x),x,a,b)"));
  quadpack_sub->Append(Event::menu_quad_qawo_cos,
                       integralSign + wxT("(f(x)*cos(ω·x),x,a,b)"));
  quadpack_sub->Append(Event::menu_quad_qaws1,
                       integralSign + wxT("(f(x)*(x-a)^α(b-x)^β,x,a,b)"));
  quadpack_sub->Append(Event::menu_quad_qaws2,
                       integralSign +
		       wxT("(f(x)*(x-a)^α(b-x)^β·log(x-a),x,a,b)"));
  quadpack_sub->Append(Event::menu_quad_qaws3,
                       integralSign +
		       wxT("(f(x)*(x-a)^α(b-x)^β·log(b-x),x,a,b)"));
  quadpack_sub->Append(
		       Event::menu_quad_qaws4,
		       integralSign + wxT("(f(x)*(x-a)^α(b-x)^β·log(x-a)·log(b-x),x,a,b)"));
  quadpack_sub->Append(Event::menu_quad_qagp,
                       integralSign +
...
#endif
  m_HelpMenu->Append(Event::menu_wxmaximahelp, _("wxMaxima help"),
                     _("The manual of wxMaxima"), wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_maximahelp, _("&Maxima help"),
                     _("The manual of Maxima"), wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_example, _("&Example..."),
                     _("Show an example of usage"), wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_apropos, _("&Apropos..."),
                     _("Show commands similar to"), wxITEM_NORMAL);
  APPEND_MENU_ITEM(m_HelpMenu, Event::menu_show_tip, _("Show &Tips..."),
                   _("Show a tip"), wxART_TIP);

  m_HelpMenu->AppendRadioItem(Event::menu_maxima_uses_internal_help,
  ...
  if (m_configuration.OfferInternalHelpBrowser())
    m_HelpMenu->AppendRadioItem(Event::menu_maxima_uses_wxmaxima_help,
                                _("Maxima shows help in a sidebar"),
                                _("Tells maxima to show the help for ?, ?? and "
                                  "describe() in a wxMaxima sidebar"));
  m_HelpMenu->AppendRadioItem(Event::menu_maxima_uses_html_help,
                              _("Maxima shows help in a browser"),
                              _("Tells maxima to show the help for ?, ?? and "
                                "describe() in a separate browser window"));
  m_HelpMenu->AppendSeparator();
  m_HelpMenu->Append(Event::menu_goto_url, _("Go to URL"));

  wxMenu *tutorials_sub = new wxMenu;
  tutorials_sub->Append(Event::menu_help_solving, _("Solving equations with Maxima"),
                        "", wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_numberformats, _("Number types"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_tolerances,
                        _("Tolerance calculations with Maxima"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_3d, _("Displaying 3d curves"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_diffequations,
                        _("Solving differential equations"), "", wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_fittingData,
                        _("Fitting curves to measurement data"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_varnames, _("Advanced variable names"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_listaccess, _("Fast list access"), "",
                        wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_memoizing, _("Memoizing"), "", wxITEM_NORMAL);
  tutorials_sub->Append(Event::menu_help_tutorials, _(wxT("↗Tutorials on the web")),
                        _("Online tutorials"), wxITEM_NORMAL);
  m_HelpMenu->Append(wxNewId(), _("Tutorials"), tutorials_sub);

  m_HelpMenu->AppendSeparator();
  m_HelpMenu->Append(Event::menu_build_info, _("Build &Info"),
                     _("Info about Maxima build"), wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_bug_report, _("&Bug Report"), _("Report bug"),
                     wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_license, _("&License"), _("wxMaxima's license"),
                     wxITEM_NORMAL);
  m_HelpMenu->Append(Event::menu_changelog, _("Change Log"), _("wxMaxima's ChangeLog"),
                     wxITEM_NORMAL);
  m_HelpMenu->AppendSeparator();
  m_HelpMenu->Append(Event::menu_check_updates, _("Check for Updates"),
...
void wxMaximaFrame::UpdateRecentDocuments() {
  ...
  for (int i = Event::menu_recent_document_0;
       i <= Event::menu_recent_document_0 + recentItems; i++) {
  ...
  for (int i = Event::menu_unsaved_document_0;
       i <= Event::menu_unsaved_document_0 + recentItems; i++) {
	...
	modified.FormatTime() + wxT(")");
      ...
      if (!separatorAdded)
        m_recentDocumentsMenu->Append(Event::menu_recent_document_separator,
  ...
  for (int i = Event::menu_recent_package_0; i <= Event::menu_recent_package_0 + recentItems;
       i++) {
...
wxWindow *wxMaximaFrame::CreateMathPane() {
  ...
  grid->Add(new wxButton(panel, Event::button_ratsimp, _("Simplify"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_radcan, _("Simplify (r)"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_factor, _("Factor"), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_expand, _("Expand"), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_rectform, _("Rectform"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_subst, _("Subst..."), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_trigrat, _("Canonical (tr)"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_trigsimp, _("Simplify (tr)"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_trigexpand, _("Expand (tr)"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_trigreduce, _("Reduce (tr)"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_solve, _("Solve..."), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_solve_ode, _("Solve ODE..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_diff, _("Diff..."), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_integrate, _("Integrate..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_limit, _("Limit..."), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_plot2, _("Plot 2D..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::button_plot3, _("Plot 3D..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
...
wxWindow *wxMaximaFrame::CreateStatPane() {
  ...
  grid1->Add(new wxButton(panel, Event::menu_stats_mean, _("Mean..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid1->Add(new wxButton(panel, Event::menu_stats_median, _("Median..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid1->Add(new wxButton(panel, Event::menu_stats_var, _("Variance..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid1->Add(new wxButton(panel, Event::menu_stats_dev, _("Deviation..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);

  box->Add(grid1, 0, style, sizerBorder);

  box1->Add(new wxButton(panel, Event::menu_stats_tt1, _("Mean Test..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  box1->Add(new wxButton(panel, Event::menu_stats_tt2, _("Mean Difference Test..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  box1->Add(new wxButton(panel, Event::menu_stats_tnorm, _("Normality Test..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  box1->Add(new wxButton(panel, Event::menu_stats_linreg, _("Linear Regression..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  box1->Add(new wxButton(panel, Event::menu_stats_lsquares, _("Least Squares Fit..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);

  box->Add(box1, 0, style, sizerBorder);

  grid2->Add(new wxButton(panel, Event::menu_stats_histogram, _("Histogram..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid2->Add(new wxButton(panel, Event::menu_stats_scatterplot, _("Scatterplot..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid2->Add(new wxButton(panel, Event::menu_stats_barsplot, _("Barsplot..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid2->Add(new wxButton(panel, Event::menu_stats_piechart, _("Piechart..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid2->Add(new wxButton(panel, Event::menu_stats_boxplot, _("Boxplot..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);

  box->Add(grid2, 0, style, sizerBorder);

  grid3->Add(new wxButton(panel, Event::menu_stats_readm, _("Read Matrix..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);
  grid3->Add(new wxButton(panel, Event::menu_stats_enterm, _("Enter Matrix..."),
                          wxDefaultPosition, wxDefaultSize),
             0, style, border);

  box->Add(grid3, 0, style, sizerBorder);

  box3->Add(new wxButton(panel, Event::menu_stats_subsample, _("Subsample..."),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
...
wxMaximaFrame::GreekPane::GreekPane(wxWindow *parent,
  ...
  Connect(wxEVT_SIZE, wxSizeEventHandler(wxMaximaFrame::GreekPane::OnSize),
          NULL, this);
  Connect(Event::menu_showLatinGreekLookalikes, wxEVT_MENU,
          wxCommandEventHandler(wxMaximaFrame::GreekPane::OnMenu), NULL, this);
  Connect(Event::menu_showGreekMu, wxEVT_MENU,
          wxCommandEventHandler(wxMaximaFrame::GreekPane::OnMenu), NULL, this);
...
void wxMaximaFrame::GreekPane::OnMenu(wxCommandEvent &event) {
  std::unordered_map<int,std::function<void()>> m{
    {Event::menu_showLatinGreekLookalikes, [&](){
      m_configuration->GreekSidebar_ShowLatinLookalikes(
							!m_configuration->GreekSidebar_ShowLatinLookalikes());
      UpdateSymbols();
      Layout();
    }},
    {Event::menu_showGreekMu, [&](){
...
void wxMaximaFrame::GreekPane::OnMouseRightDown(wxMouseEvent &WXUNUSED(event)) {
  ...
  popupMenu->AppendCheckItem(Event::menu_showLatinGreekLookalikes,
                             _(wxT("Show greek \u21D4 latin lookalikes")));
  popupMenu->Check(Event::menu_showLatinGreekLookalikes,
                   m_configuration->GreekSidebar_ShowLatinLookalikes());
  popupMenu->AppendCheckItem(Event::menu_showGreekMu,
                             _(wxT("Show lookalike for unit prefix µ")));
  popupMenu->Check(Event::menu_showGreekMu, m_configuration->GreekSidebar_Show_mu());
  PopupMenu(&*popupMenu);
}
...
wxMaximaFrame::SymbolsPane::SymbolsPane(wxWindow *parent,
  ...
  Connect(wxEVT_SIZE, wxSizeEventHandler(wxMaximaFrame::SymbolsPane::OnSize),
          NULL, this);
  Connect(Event::menu_additionalSymbols, wxEVT_MENU,
...
void wxMaximaFrame::SymbolsPane::OnMouseRightDown(
  ...
  std::unique_ptr<wxMenu> popupMenu(new wxMenu());
  popupMenu->Append(Event::menu_additionalSymbols, _("Add more symbols"),
                    wxEmptyString, wxITEM_NORMAL);
  popupMenu->Append(Event::enable_unicodePane, _("Show all unicode symbols"),
                    wxEmptyString, wxITEM_NORMAL);
  PopupMenu(&*popupMenu);
}
...
wxWindow *wxMaximaFrame::CreateFormatPane() {
  ...
  grid->Add(new wxButton(panel, Event::menu_format_text, _("Text"), wxDefaultPosition,
                         wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_title, _("Title"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_section, _("Section"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_subsection, _("Subsection"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_subsubsection, _("Subsubsection"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_heading5, _("Heading 5"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_heading6, _("Heading 6"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_image, _("Image"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
  grid->Add(new wxButton(panel, Event::menu_format_pagebreak, _("Pagebreak"),
                         wxDefaultPosition, wxDefaultSize),
            0, style, border);
...
wxMaximaFrame::DrawPane::DrawPane(wxWindow *parent, int id)
  ...
  m_grid->Add(m_draw_setup2d = new wxButton(this, Event::menu_draw_2d, _("2D"),
                                            wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_setup2d->SetToolTip(_("Setup a 2D plot"));
  m_grid->Add(m_draw_setup3d = new wxButton(this, Event::menu_draw_3d, _("3D"),
                                            wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_setup3d->SetToolTip(_("Setup a 3D plot"));
  m_grid->Add(m_draw_explicit =
	      new wxButton(this, Event::menu_draw_explicit, _("Expression"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_explicit->SetToolTip(
			      _("The standard plot command: Plot an equation as a curve"));
  m_grid->Add(m_draw_implicit =
	      new wxButton(this, Event::menu_draw_implicit, _("Implicit Plot"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_grid->Add(m_draw_parametric =
	      new wxButton(this, Event::menu_draw_parametric, _("Parametric Plot"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_grid->Add(m_draw_points = new wxButton(this, Event::menu_draw_points, _("Points"),
                                           wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_grid->Add(m_draw_title =
	      new wxButton(this, Event::menu_draw_title, _("Diagram title"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_title->SetToolTip(_("The diagram title"));
  m_grid->Add(m_draw_axis = new wxButton(this, Event::menu_draw_axis, _("Axis"),
                                         wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_axis->SetToolTip(_("Setup the axis"));
  m_grid->Add(m_draw_contour =
	      new wxButton(this, Event::menu_draw_contour, _("Contour"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_grid->Add(m_draw_key = new wxButton(this, Event::menu_draw_key, _("Plot name"),
                                        wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_key->SetToolTip(_("The next plot's title"));
  m_grid->Add(m_draw_fgcolor =
	      new wxButton(this, Event::menu_draw_fgcolor, _("Line color"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_fgcolor->SetToolTip(_("The color of the next line to draw"));
  m_grid->Add(m_draw_fillcolor =
	      new wxButton(this, Event::menu_draw_fillcolor, _("Fill color"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
  m_draw_fillcolor->SetToolTip(_("The fill color for the next objects"));
  m_grid->Add(m_draw_grid = new wxButton(this, Event::menu_draw_grid, _("Grid"),
                                         wxDefaultPosition, wxDefaultSize),
              0, style, border);
  ...
  m_grid->Add(m_draw_accuracy =
	      new wxButton(this, Event::menu_draw_accuracy, _("Accuracy"),
			   wxDefaultPosition, wxDefaultSize),
              0, style, border);
